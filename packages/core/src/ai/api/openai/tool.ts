...
export function tryCallToolFunction(
  ...
  try {
    const functionToCall = toolFunctions[functionData.name as keyof typeof toolFunctions]
...
const getCurrentWeather: ToolFunction<{ location: string; unit?: string }> = (data) => {
  const { location } = data
