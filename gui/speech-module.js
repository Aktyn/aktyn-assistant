  ...
  (function () {
    ...
    function init(lang_code) {
      setVolume = (function () {
        ...
        const volume_settings = document.getElementById('volume-settings');

        const mute_btn = document.getElementById('mute-btn');
        if (mute_btn) mute_btn.onclick = () => setVolume(0);
        const volume_max_btn = document.getElementById('volume-max-btn');
        if (volume_max_btn) volume_max_btn.onclick = () => setVolume(1);

        const VOLUME_STEP = 0.2;

        const volume_down = document.getElementById('volume-down');
        if (volume_down) volume_down.onclick = () => setVolume(Math.max(0, volume - VOLUME_STEP));
        const volume_up = document.getElementById('volume-up');
        if (volume_up) volume_up.onclick = () => setVolume(Math.min(1, volume + VOLUME_STEP));
        ...
        return function (vol) {
          ...
          if (volume_settings) {
            if (volume > 0.001) volume_settings.classList.remove('muted');
            else volume_settings.classList.add('muted');
          }
        };
      })();
      ...
      recognition.onstart = () => {
        ...
        if (typeof _module.onstart === 'function') _module.onstart();
      };
      recognition.onend = () => {
        if (recognition_active) {
          recognition_active = false;
          if (Date.now() - recognition_start_timestamp > 1000) {
        ...
        if (typeof _module.onend === 'function') _module.onend();
      };
      ...
      recognition.onresult = async event => {
        ...
        if (ignore_index === event.resultIndex) {
          //recognition already succeeded
