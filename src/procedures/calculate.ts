...
const keywords_replacements = new Map<string, ((input: string) => string) | RegExp[]>([
  ...
  [
    operations.power.symbol,
    input => {
      const match = input.match(/do ([^ ]+) pot[eę]gi/);
      if (match) {
  ...
  [
    operations.cosinus.symbol,
    input => {
      const match = input.match(/cosinus (\d+[.,]?\d*) stopni/);
      return match
        ? `${input.substring(0, match.index)} cos(${match[1]}) ${input.substr((match.index || 0) + match[0].length)}`
        : input;
    }
  ],
  [
    operations.sinus.symbol,
    input => {
      //sinus must be after cosinus because word "cosinus" contains "sinus"
      const match = input.match(/sinus (\d+[.,]?\d*) stopni/);
      return match
        ? `${input.substring(0, match.index)} sin(${match[1]}) ${input.substr((match.index || 0) + match[0].length)}`
        : input;
    }
  ],
  [
    operations.tangent.symbol,
    input => {
      const match = input.match(/tangens (\d+[.,]?\d*) stopni/);
      return match
        ? `${input.substring(0, match.index)} tan(${match[1]}) ${input.substr((match.index || 0) + match[0].length)}`
        : input;
    }
  ],
...
export class Calculate extends ProcedureBase {
  ...
  update(results: ResultSchema[]) {
    if (results.some(res => res.type === RESULT_TYPE.INTERIM))
      //process only final or alternative results
      return;

    //get most confident sentence that is math command
    const equation_sentence = results
      .filter(res => Calculate.isMathCommand(res.result))
      .sort((r1, r2) => r2.confidence - r1.confidence)[0];
    if (!equation_sentence) return;

    let formatted_sentence = equation_sentence.result; //.replace(/\s/g, '');

    for (const [replacement, search_regexp] of keywords_replacements.entries()) {
      if (typeof search_regexp === 'function') formatted_sentence = search_regexp(formatted_sentence);
      else {
        for (const regexp of search_regexp)
    ...
    const symbols_regexp = new RegExp(
      [/\d+[.,]?\d*/.source, /\(/.source, /\)/.source, ...operation_symbols].join('|'),
      'gi'
    );
    //console.log('test', symbols_regexp);
    formatted_sentence = (formatted_sentence.match(symbols_regexp) || []).join('');

    const result = calculateInfix(formatted_sentence);
    const equation = formatted_sentence + ' = ' + result;
