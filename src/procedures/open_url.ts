...
const predefined = new Map([
  //NOTE: regexps should have 'i' flag
...
export class OpenUrl extends ProcedureBase {
  ...
  update(results: ResultSchema[]) {
    ...
    for (const { result } of results.sort((r1, r2) => r2.confidence - r1.confidence)) {
      const url_match = result.trim().match(/([^. /:]+[. ][a-z]{2,3})( ?uko[sś]nik ?.+| ?slash ?.+)*$/i);
      if (!url_match || !url_match[0]) {
        //try predefined website
        for (const [regexp, href] of predefined.entries()) {
