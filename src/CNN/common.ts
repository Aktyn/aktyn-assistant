...
export function getImageTensor(images_data: Float32Array, elements: number, image_info: ImageInfoSchema) {
  ...
  return tf
    .tensor2d(images_data, [elements, image_size])
    .reshape([elements, image_info.width, image_info.height, image_info.channels]);
}

export function getDataTensors(
  images_data: Float32Array,
  labels_data: Uint8Array,
  elements: number,
...
export function train(
  ...
  train_data: DataSchema,
  test_data: DataSchema,
  epochs: number,
  batchSize = 128
) {
  ...
  const [trainXs, trainYs] = tf.tidy(() => {
    return [train_data.pixels, train_data.labels];
  });

  const [testXs, testYs] = tf.tidy(() => {
    return [test_data.pixels, test_data.labels];
  });
...
export function predict(model: tf.LayersModel, input_data: tf.Tensor<tf.Rank>) {
  return <Float32Array>(<tf.Tensor>model.predict(input_data)) /*.argMax(-1)*/
    .dataSync();
}
