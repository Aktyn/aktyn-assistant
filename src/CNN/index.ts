...
import { getImageTensor, LayersModel, loadModel, predict } from './common';
...
export async function classifyDesktopContent() {
  ...
  const screenshot_buffer = Buffer.alloc(screenshot.width * screenshot.height * 3);
  ...
  const channels: 4 | 3 = 3;
  const opts = {
    raw: { width: screenshot.width, height: screenshot.height, channels: channels }
  };
  const buffer = await sharp(screenshot_buffer, opts).resize(IMAGE_INFO.width, IMAGE_INFO.height).raw().toBuffer();
  ...
  const data = new Float32Array(IMAGE_SIZE); //RGB array

  assert(buffer.length === channels * IMAGE_INFO.width * IMAGE_INFO.height, 'Incorrect buffer size');
  let data_i = 0;
  for (let i = 0; i < buffer.length; i += channels) {
    for (let c = 0; c < IMAGE_INFO.channels; c++) data[data_i++] = buffer[i + c] / 255.0;
  }

  assert(data_i === data.length, 'Image data has not been loaded correctly');

  /////////////////////////////////////////////////////
  if (!model) model = await loadModel(path.join(models_dir, 'screenshot_classification', 'model.json'));

  const input_tensor = getImageTensor(data, 1, IMAGE_INFO);
  const prediction = Array.from(predict(model, input_tensor));
