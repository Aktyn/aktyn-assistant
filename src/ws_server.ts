...
export function init(port: number) {
	if(open_port)
		throw new Error('WebSocket server already runs at ws_port ' + open_port);
	open_port = port;
	...
	websocket.on('connection', function(ws) {
		...
		let connection = new ListenerConnection(ws);
	...
	setInterval(function ping() {
		websocket.clients.forEach((ws) => {
			if ((<any>ws).isAlive === false) {//connection didn't send pong in time
