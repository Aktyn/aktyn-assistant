...
export interface Prediction {
  ...
  annotations?: { [index: string]: [number, number, number][] };
}
...
export class HandPipeline {
  ...
  constructor(
    boundingBoxDetector: HandDetector,
    meshDetector: tfconv.GraphModel,
    meshWidth: number,
    meshHeight: number,
  ...
  private getBoxForPalmLandmarks(palmLandmarks: Coords2D, rotationMatrix: TransformationMatrix): Box {
    const rotatedPalmLandmarks: Coords2D = palmLandmarks.map((coord: [number, number]): [number, number] => {
  ...
  private getBoxForHandLandmarks(landmarks: Coords3D): Box {
    ...
    for (let i = 0; i < PALM_LANDMARK_IDS.length; i++) {
      palmLandmarks.push(landmarks[PALM_LANDMARK_IDS[i]].slice(0, 2) as [number, number]);
    }
  ...
  private transformRawCoords(
    ...
    const coordsRotated = coordsScaled.map(coord => {
      ...
      return [...rotated, coord[2]] as [number, number, number];
    });

    const inverseRotationMatrix = invertTransformMatrix(rotationMatrix);
    const boxCenter = [...getBoxCenter(box), 1];

    const originalBoxCenter = [dot(boxCenter, inverseRotationMatrix[0]), dot(boxCenter, inverseRotationMatrix[1])];

    return coordsRotated.map((coord): [number, number, number] => {
      return [coord[0] + originalBoxCenter[0], coord[1] + originalBoxCenter[1], coord[2]];
    });
  }

  async estimateHand(image: tf.Tensor4D): Promise<Prediction | null> {
    ...
    const rotatedImage = image; //tf.image.rotateWithOffset(image, angle, 0, palmCenterNormalized);
    ...
    // the palm keypoints.
    if (useFreshBox === true) {
      box = this.getBoxForPalmLandmarks(currentBox.palmLandmarks!, rotationMatrix);
    } else {
      box = currentBox;
    }
    ...
    if (tf.getBackend() === 'webgl') {
      ...
      prediction = this.meshDetector.predict(handImage) as [tf.Tensor, tf.Tensor];
