...
export function getColumnFrom2DArr(arr: number[][], columnIndex: number): number[] {
  const column: number[] = [];
...
function multiplyTransformMatrices(mat1: number[][], mat2: number[][]): TransformationMatrix {
...
export function buildRotationMatrix(rotation: number, center: [number, number]): TransformationMatrix {
...
export function invertTransformMatrix(matrix: TransformationMatrix): TransformationMatrix {
  ...
  return [
    rotationComponent[0].concat(invertedTranslation[0]) as [number, number, number],
    rotationComponent[1].concat(invertedTranslation[1]) as [number, number, number],
    [0, 0, 1]
  ];
}
