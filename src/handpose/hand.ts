...
export class HandDetector {
  ...
  private normalizeBoxes(boxes: tf.Tensor2D): tf.Tensor2D {
    return tf.tidy(() => {
      ...
      const startPoints: tf.Tensor2D = tf.mul(tf.sub(boxCenterPoints, halfBoxSizes), this.inputSizeTensor);
      const endPoints: tf.Tensor2D = tf.mul(tf.add(boxCenterPoints, halfBoxSizes), this.inputSizeTensor);
      return tf.concat2d([startPoints, endPoints], 1);
    });
  }

  private normalizeLandmarks(rawPalmLandmarks: tf.Tensor2D, index: number): tf.Tensor2D {
    return tf.tidy(() => {
  ...
  private async getBoundingBoxes(input: tf.Tensor4D): Promise<HandDetectorPrediction | null> {
    ...
    const scores: tf.Tensor1D = tf.tidy(() => tf.sigmoid(tf.slice(prediction, [0, 0], [-1, 1])).squeeze());
   ...
   */
  async estimateHandBounds(input: tf.Tensor4D): Promise<Box | null> {
    const inputHeight = input.shape[1];
    const inputWidth = input.shape[2];

    const image: tf.Tensor4D = tf.tidy(() => input.resizeBilinear([this.width, this.height]).div(255));
    ...
    const boundingBoxes = prediction.boxes.arraySync() as Array<[number, number, number, number]>;
    const startPoint = boundingBoxes[0].slice(0, 2) as [number, number];
    const endPoint = boundingBoxes[0].slice(2, 4) as [number, number];
    const palmLandmarks = prediction.palmLandmarks.arraySync() as Array<[number, number]>;

    image.dispose();
    prediction.boxes.dispose();
    prediction.palmLandmarks.dispose();

    return scaleBoxCoordinates({ startPoint, endPoint, palmLandmarks }, [
