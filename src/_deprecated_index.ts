...
export { langCode } from './procedures/common/lang_codes';
...
// noinspection JSUnusedGlobalSymbols
async function init(use_procedures: procedure[], params: Partial<ConfigSchema> = {}) {
  ...
  const chrome_args =
    `--chrome-frame --app-shell-host-window-size=256x414 --ash-host-window-bounds=256x414` +
  ...
  if (typeof config.express_port !== 'number') {
    ...
    if (!config.open_listener) {
      console.log(
        `You can manually open listener in chrome browser with this link: ${app_value}\n` +
          `or if you're connecting from different location: ${app_value.replace('localhost', ip.address())}`
      );
    }
  }
