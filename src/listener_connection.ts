...
let id_counter = 0;
const active_connections = new Map<number, ListenerConnection>();
...
export default class ListenerConnection {
  ...
  private handleMessage(msg: { [index: string]: any }) {
    //[{result, confidence, type}], index
    try {
      if (!Array.isArray(msg.results) || typeof msg.index !== 'number') return this.send({ res: 'incorrect input' });
      const response: { res: string; notify?: NotificationParams } = parseResult(msg.results, msg.index);
