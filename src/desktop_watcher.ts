...
function readLabelValues() {
	...
	labels.split('\n').forEach((line, index) => {
		...
		assert(values.length === CATEGORIES.COUNT,
	...
	assert(files.length*CATEGORIES.COUNT === label_values.length,
		`Number of label values must be ${CATEGORIES.COUNT} times larger than number of files`);
...
/*async function temp() {
	...
	for(let f of to_remove) {
		console.log('removing:', f);
		fs.unlinkSync(path.join(dataset_dir, f));
	}
}*/

async function trainBatch(model: CNN.LayersModel, dataset: {files: string[], label_values: Uint8Array},
                          start: number, end: number, test_data: CNN.DataSchema)
{
	const elements = end-start;
	
	// PREPARE TRAIN DATA
	const train_image_data = await loadImagesData( dataset.files.slice(start, end) );
	const train_label_values = dataset.label_values.slice(CATEGORIES.COUNT*start, CATEGORIES.COUNT*end);
	
	assert(train_image_data.length === elements*IMAGE_SIZE,
		'Train image data extracted incorrectly');
	assert(train_label_values.length === elements*CATEGORIES.COUNT,
		'Train label values extracted incorrectly');
	
	const train_data = CNN.getDataTensors(
		train_image_data, train_label_values,
		elements, IMAGE_INFO, CATEGORIES.COUNT
	);
	...
	// VALIDATE
	let prediction: number[] = Array.from( CNN.predict(model, test_data.pixels) );
	
	let expected_predictions = test_data.labels.dataSync();
	
	for(let i=0; i<expected_predictions.length; i+=CATEGORIES.COUNT) {
		console.log(
			prediction.slice(i, i+CATEGORIES.COUNT).map(v => v.toFixed(2)).join(', '),
			'\t|\t',
			expected_predictions.slice(i, i+CATEGORIES.COUNT) .join(', ')
		);
	}
}
...
export async function watch() {
	...
	let shuffled_files: string[] = new Array(dataset.files.length);
	...
	for(let i=0; i<indices.length; i++) {
		...
		shuffled_label_values.set(
			dataset.label_values.slice(CATEGORIES.COUNT*idx, CATEGORIES.COUNT*(idx+1)),
			i*CATEGORIES.COUNT
		);
	}
	dataset = {
	...
	const test_label_values = dataset.label_values.slice(CATEGORIES.COUNT*train_elements);
	
	assert(test_image_data.length === test_elements*IMAGE_SIZE,
		'Test image data extracted incorrectly');
	assert(test_label_values.length === test_elements*CATEGORIES.COUNT,
		'Test label values extracted incorrectly');
	
	const test_data = CNN.getDataTensors(
		test_image_data, test_label_values,
		test_elements, IMAGE_INFO, CATEGORIES.COUNT
	);
	
	// LOAD OR BUILD MODEL FOR TRAINING
	//const model = CNN.buildModel(IMAGE_INFO, CATEGORIES.COUNT);
